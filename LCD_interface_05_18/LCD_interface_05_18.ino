...
int cycleCount = CYCLEMAX-10;
double fanLevel = 0;
double setpoint = 65;
double setRead,d0=75.0/CYCLEMAX,i1=0,p1=0,d1=0;
...
//      myServo.write(servoLevel);
  ...
  internalTemp = (internalTemp+externalTemp)/2;
...
void checkActivationState(void) {
  if (cycleCount == 0) {
    fanLevel = I_WEIGHT*i1+P_WEIGHT*p1-D_WEIGHT*(d1-d0);
    d0 = d1;
    d1 = 0;
    p1 = 0;
    if (fanLevel>=30) fanLevel = 255;
    if (fanLevel<30) fanLevel = 0;
    coolingOn = fanLevel>0;
    digitalWrite(FAN,coolingOn);
  }
